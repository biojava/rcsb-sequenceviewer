...
public class ColorWheelUtil {
    ...
    public static int colorWheelCounter = 0;
    ...
    static {
        ...
        COLORS = new Color[]{pdbRED, pdbRED2, pdbPURPLE, pdbBLUE, pdbBLUE2, pdbGREEN, pdbGREEN2, pdbYELLOW};
    }
    public static Map<Object, Color> colorMap = new HashMap<Object, Color>();

    public static Color getArbitraryColor(int aNumber) {

        Color c = COLORS[colorWheelCounter];
        //System.out.println("getting color " + colorWheelCounter + " " + c) ;
        colorWheelCounter++;
        if (colorWheelCounter >= COLORS.length)
            colorWheelCounter = 0;
